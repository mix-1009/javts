...
class VTReportOutputGenerator:
    ...
    def _print_results(self, report, entities):
        print('\n')
        print('{:#^64}'.format(''))
        print('{:^64}'.format('Virus Total Report {0}'.format(
        ...
        if VTReportOutputGenerator.VERBOSE_MODE:
            for v in (positives+clear):
                self._print(v.request_result, entities[v.original_entity])
            for v in empty:
                print('{0:-^64}\n{1} : {2}\n'.format(
                    entities[v.original_entity], v.request_result['resource'],
                    v.request_result['verbose_msg']))
            for v in bad_requests:
                print('{0:-^64}\n{1}'.format(
                    entities[v.original_entity],
                    v.request_result['verbose_msg']))

        print('{:#^64}'.format(' Statistics '))
        print('Total entities was requested: {}'.format(len(entities)))
        print('Total reports was received: {}'.format(
            len(positives)+len(clear)))
        print('  positives scans: {}'.format(len(positives)))
        print('  clean scans: {}'.format(len(clear)))
        print('Not among the finished, queued or pending scans: {}'.format(
            len(empty)))
        if len(bad_requests) > 0:
            print('Responces FAILED: {}, it have sense to try again.'.format(
                len(bad_requests)))
    ...
    def _print(self, file_report, origin_entity):
        ...
        print('Rate: {0}/{1}'.format(
            file_report['positives'], file_report['total']))
        ...
        for av, results in file_report['scans'].items():
            if results['detected'] is True:
                print(' - {0} ({1} {2}): {3}'.format(
                    av, results['version'], results['update'],
                    results['result']))
        print('\n')
